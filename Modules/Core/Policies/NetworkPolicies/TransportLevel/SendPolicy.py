...
class STDTransportLevelSendPolicy(AbstractTransportLevelSendPolicy):
    ...
    def connect(self):
        try:
            ...
            self._logger.debug(MODULE_PREFIX, "Успешное подключение к", self._receiver.get_object_data())
        except ConnectionRefusedError:
            self._logger.debug(MODULE_PREFIX, "Подключение отклонено, пробую еще раз...")
            self.connect()

    # TODO: Проверять отправленное кол-во байт и навесить исключение
    def send_data(self, data):
        self._logger.debug(MODULE_PREFIX, "Получатель:", self._receiver.get_object_data(),
    ...
    def disconnect(self):
        self._socket.close()
        self._logger.debug(MODULE_PREFIX, "Подключение", self._receiver.get_object_data(), "закрыто")

    def get_data(self, data_size):
        self._logger.debug(MODULE_PREFIX, "Получаю данные от", self._receiver.get_object_data())
        data = self._socket.recv(data_size)
        self._logger.debug(MODULE_PREFIX, "Получено", data_size, "байта данных от", self._receiver.get_object_data())
        return data
