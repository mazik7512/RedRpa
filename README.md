...
- [Описание](#описание)
  ...
  * [Ядро. Криптография](#ядро-криптография)
     + [Ядро. Криптография. Шифрование](#ядро-криптография-шифрование)
     + [Ядро. Криптография. Хэширование](#ядро-криптография-хэширование)
  * [Ядро. Работа с сетью](#ядро-работа-с-сетью)
     + [Ядро. Работа с сетью. Сетевые менеджеры](#ядро-работа-с-сетью-сетевые-менеджеры)
     + [Ядро. Работа с сетью. Сетевые протоколы](#ядро-работа-с-сетью-сетевые-протоколы)
...
![Static Badge](https://img.shields.io/badge/ВАЖНО-red) *В секции импорта определяются только модули требуемые для выполнения REX, пути их импорта вычисляются непосредственно перед выполнением внутри RVM*.
...
Здесь может представлять интерес только операция "Обновление сессионых ключей" данный протокол предназначен для запуска алгоритма описанного ранее в пункте [Шифрование](#ядро-криптография-шифрование), начиная с пункта 2.
...
**Политика же уровня приложения** будет определять формат подключений и обмена данными и опять же может быть имплементирована в виде HTTP, но уже не получится использовать чистый TCP, ведь он просто не располагает такими возможностями. По умолчанию политика транспортного уровня определяет формат подключения как защищенный канал (описанный в главе [«Сетевые менеджеры»](#ядро-работа-с-сетью-сетевые-менеджеры)), а также определяет формат обмена размерами посылаемых сообщений протокола – размер посылается в незашифрованном виде в первых четырех байтах сообщения.
...
![Static Badge](https://img.shields.io/badge/Guideline-purple)
...
![Static Badge](https://img.shields.io/badge/Release-aqua)
...
[Демонстрация](https://github.com/mazik7512/RedRpa/assets/45432633/c1a5b7c0-16ad-4f52-9831-927ad62685a5)
