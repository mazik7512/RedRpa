...
**ВАЖНО!** *В секции импорта определяются только модули требуемые для выполнения REX, пути их импорта вычисляются непосредственно перед выполнением внутри RVM*.
