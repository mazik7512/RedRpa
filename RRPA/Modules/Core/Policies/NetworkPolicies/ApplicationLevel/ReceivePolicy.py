...
class STDAppLevelReceivePolicy(AbstractAppLevelReceivePolicy):
    ...
    def __init__(self, TransportLevelReceivePolicy: AbstractTransportLevelReceivePolicy, logger: AbstractLogger):
    ...
    def wait_for_session(self, data):
        self._logger.info(MODULE_PREFIX, "Ожидание начала сессии связи")
        self._tlr_policy.start_listen()
        self._logger.info(MODULE_PREFIX, "Инициализация сессии связи")
        self._send_rsa_key(data)

    def _send_rsa_key(self, rsa_key):
        self.send_data(rsa_key)
        self._logger.info(MODULE_PREFIX, "RSA-ключ отправлен")

    def end_session(self):
        self._tlr_policy.close_connection()
        self._logger.info(MODULE_PREFIX, "Сессия связи завершена")
    ...
    def send_data(self, data):
        ...
        self._logger.info(MODULE_PREFIX, "Процедура отправки пакета завершена")

    def stop_waiting(self):
        self._tlr_policy.stop_listen()
        self._logger.info(MODULE_PREFIX, "Ожидание сессий связи завершено")
